...
(sum((x - mean)^3)/n)/(sum((x - mean)^2)/n) ^ 3/2

$$Skew=\sum_{t=1}^n (x_i-\overline{x})^3/n/(\sum_{t=1}^n (x_i-\overline{x})^2/n)^{3/2} $$

Notice how standard deviation gives weight but not special weight to huge outliers, like those massive heart attacks. Skew pays more attention to those and a high skew alerts us to look for and explain those outlying events. 
